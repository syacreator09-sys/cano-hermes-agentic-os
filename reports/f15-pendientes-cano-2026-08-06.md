...
| 10 | ~~6 servicios sin ninguna llave en vault~~ **REDIRIGIDO 2026-08-06 — los 6 SÍ están conectados, vía MCP de Claude.ai, no vault** | `connection_matrix.py` solo audita `.env`, es ciego a MCP — quedó corregido este hallazgo verificando cada uno hoy (solo lectura): **Shopify** → tienda real "CASS Beauty Clinic" (`fss1nv-s1.myshopify.com`, plan Basic, MXN). **Vercel** → team `syacreator09-sys-projects`, 3 proyectos (`web`, `worldvibe` — solo lectura, sigue intocable para escritura, `luzya-strategy-hub`). **Gamma** → cuenta activa con 5+ documentos reales ya creados. **Adobe** → cuenta autenticada completa (`account_type: auth`). **Canva** → conector activo, 0 brand kits configurados (posible pendiente menor: crear un brand kit si se quiere generar con identidad de marca). **Meta** → ver #9. Ninguno de estos MCP tiene wiring hacia StarHome/Hermes todavía (son solo accesibles desde una sesión de Claude.ai, no desde `ExecutionService`/`office-*`) — diseñar esa conexión es trabajo nuevo, no un pendiente de credenciales | Ninguna acción de Cano para "conectar" — ya están. Si se quiere brand kit en Canva, es la única atomicidad real pendiente | connection_matrix (F2/F15-it1), redirigido y verificado hoy |
...
| 12 | Credenciales de `amazon-fba-product-hunter` — **PRIORIZADA por Cano 2026-08-06** | `CONTARMARKET_*`, `FLYBY_API_KEY`, `KEEPA_API_KEY`, `AMAZON_SP_API_*` — pendientes de alta | Alta de cuenta/API key por proveedor | F-anterior, priorizado hoy |
| 17 | `factory-ia-channel-v5` — Apify/Supadata/UploadPost verificados 2026-08-06 (Cano tenía razón, ya estaba casi todo) | **Apify**: ✅ ya conectado y verificado en vivo hoy (`providers/apify_healthcheck.py`, `status: configured, live_checked: true` contra la API real). **Supadata**: proveedor completo ya en el repo (`providers/supadata/client.py`, cache, error mapper) pero `SUPADATA_API_KEY` no existe en NINGÚN vault — reporta `not_configured` correctamente, no es un bug, falta la llave. **UploadPost**: cuenta ya validada en F12 vía MCP (4/4 perfiles); el bridge de este repo (`providers/upload_post_bridge.py`) se queda intencionalmente en modo `manual_required`/dry-run por regla dura del propio `CLAUDE.md` de factory-v5 ("UploadPost is manual-required/dry-run only here; never claim it completed a live upload") — no hay nada que "arreglar" ahí, es diseño correcto. **Diseños (Canva/Gamma/Adobe)**: sin código de bridge en este repo bajo ningún nombre buscado — el patrón que sí existe es el de ImageGen nativa (`docs/cano/NATIVE-IMAGEGEN-BRIDGE.md`, no encontrado en este repo extraído, solo referenciado en `CLAUDE.md`) — construir el equivalente para Canva/Gamma/Adobe es diseño nuevo | Solo falta: dar de alta `SUPADATA_API_KEY` con el proveedor | Verificado hoy con código y llamadas reales |
